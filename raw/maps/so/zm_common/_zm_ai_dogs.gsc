...
dog_round_aftermath()
{
	...
	if( IsDefined( power_up_origin ) )
	{
		...
		level thread maps\so\zm_common\_zm_powerups::powerup_drop( power_up_origin );
...
add_dog_dialog(player)
{
	...
	{
		num_variants = maps\so\zm_common\_zm_audio::get_number_variants(player_index + "vox_dog_spawn");
	...
	player maps\so\zm_common\_zm_audio::do_player_playdialog(player_index, sound_to_play, 0.25);
}
...
dog_death()
{
	...
	{
		self.attacker maps\so\zm_common\_zm_score::player_add_points( "death", self.damagemod, self.damagelocation, true );
	}

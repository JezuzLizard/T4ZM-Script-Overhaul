...
tesla_do_damage( source_enemy, arc_num, player )
{
	...
	player maps\so\zm_common\_zm_score::player_add_points( "death", "", "" );
	...
	if ( !player.tesla_powerup_dropped && player.tesla_enemies_hit >= level.zombie_vars["tesla_kills_for_powerup"] )
	{
		...
		level thread maps\so\zm_common\_zm_powerups::powerup_drop( self.origin );
	}
}
...
tesla_killstreak_sound()
{
	...
	plr = "plr_" + index + "_";
	self thread maps\so\zm_common\_zm_audio::play_tesla_dialog(plr);	
	wait(3.5);
